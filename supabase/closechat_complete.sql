-- CloseChat completo: executar de uma só vez no SQL Editor de um projeto Supabase novo.
-- Inclui autenticação/perfis, amigos, conversas, imagens, áudio, localização,
-- stories, notificações push, Storage, RLS e Realtime.
create extension if not exists pgcrypto;

create type public.friend_request_status as enum ('pending','accepted','declined','cancelled');
create type public.message_kind as enum ('text','image','voice','location');
...
create policy "stories visible to friends" on public.stories for select to authenticated using(user_id=auth.uid() or public.are_friends(auth.uid(),user_id));
create policy "stories create own" on public.stories for insert to authenticated with check(user_id=auth.uid() and expires_at<=now()+interval '24 hours 5 minutes');
create policy "stories delete own" on public.stories for delete to authenticated using(user_id=auth.uid());
...
create policy "voice conversation member upload" on storage.objects for insert to authenticated with check(bucket_id='chat-voice' and public.is_conversation_member(((storage.foldername(name))[1])::uuid));
create policy "voice conversation member read" on storage.objects for select to authenticated using(bucket_id='chat-voice' and public.is_conversation_member(((storage.foldername(name))[1])::uuid));
create policy "voice owner delete" on storage.objects for delete to authenticated using(bucket_id='chat-voice' and owner_id=auth.uid()::text);
